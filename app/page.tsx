...
import {
  type EventPlanData,
  useBookingPanel,
} from "@/components/booking-panel-context";
...
import type { WorkflowInsight } from "@/lib/workflow-lang";
...
type PlanToolPart = {
  type: "tool-create_event_plan";
  state: PlanToolState;
  input?: PlanToolInput;
  output?: PlanToolOutput;
};

type InsightsToolPart = {
  type: "tool-add_workflow_insights";
  input?: { insights?: WorkflowInsight[] };
};
...
export default function ChatPage() {
  ...
  // Derive the live workflow from the most recent create_event_plan tool call.
  const planPart = useMemo(() => {
    for (let i = messages.length - 1; i >= 0; i--) {
      const part = messages[i].parts.find(
        (p) => p.type === "tool-create_event_plan",
      );
      if (part) return part as unknown as PlanToolPart;
    }
    return undefined;
  }, [messages]);
  ...
  const insights = useMemo<WorkflowInsight[]>(() => {
    for (let i = messages.length - 1; i >= 0; i--) {
      const part = messages[i].parts.find(
        (p) => p.type === "tool-add_workflow_insights",
      );
      if (part) {
        return (part as unknown as InsightsToolPart).input?.insights ?? [];
      }
    }
    return [];
  }, [messages]);
